...
## 2026-09-20 19:04 — Claude
Estado: en progreso
Resumen: se agrego una segunda capa de seguridad al server.js: dado que Cloudways Velocity no da acceso SSH, se extrajo el contenido de prueba a scripts/datos-ejemplo.js (funcion cargarDatosEjemplo) para que el servidor la cargue solo al arrancar si la base esta vacia (no duplica si ya hay datos); scripts/seed-datos-ejemplo.js quedo como wrapper delgado para uso manual en desarrollo. Se creo la app en Cloudways (Node v24, deploy automatico desde github.com/sergioaraos/fotografia rama main) y se configuro el dominio fotografia.araos.net. Troubleshooting de DNS/SSL: el dominio se movio de Porkbun a Cloudflare como DNS autoritativo; el primer intento de CNAME quedo con proxy de Cloudflare activado (nube naranja), lo que impedia que Cloudways emitiera su propio certificado SSL (se veia el certificado por defecto de HostGator); se corrigio dejando el registro en "DNS only" (nube gris). Ademas la VPN de Sergio (Surfshark) tenia una respuesta DNS vieja cacheada y mostraba una pagina 404, cosa que no ocurria desde otras ubicaciones (DNS ya propagado globalmente segun dnschecker.org). Al final Cloudways emitio solo un certificado SSL valido (Google Trust Services) para fotografia.araos.net. Pendiente que Sergio confirme que https://fotografia.araos.net carga sin advertencias del certificado.
Archivos/carpetas tocados: server.js, scripts/datos-ejemplo.js (nuevo), scripts/seed-datos-ejemplo.js
Recursos/URLs/config: app en Cloudways con dominio fotografia.araos.net; DNS de araos.net administrado en Cloudflare (registro CNAME fotografia -> secure.cloudwaysnodeapps.com, en modo DNS only); certificado SSL de Google Trust Services emitido el 2026-09-20, vence 2026-12-19
Pendientes: confirmar que fotografia.araos.net carga bien con HTTPS desde el navegador de Sergio (sin VPN y con VPN). Despues: verificar que un push a main dispare el auto-deploy en Cloudways, y luego reemplazar los datos de prueba por el contenido real del capitulo 1. Pendiente tambien Lightroom Classic y modo escena de la Z5 II.
