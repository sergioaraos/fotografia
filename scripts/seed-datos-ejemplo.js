// SERGIO 2026-09-20: script reutilizable para cargar datos de ejemplo en data/aprender_fotografia.db
// cuando esta vacia (feature/port-a-nodejs). Mismo contenido que se habia cargado originalmente
// en la version Python (database.py), portado 1:1. No inserta nada si ya hay capitulos, para
// no duplicar datos. Se corre a mano con "npm run seed"; sirve tanto para un ambiente de
// desarrollo nuevo como para la primera carga en un despliegue nuevo (por ejemplo Cloudways),
// mientras no se haya cargado el contenido real de los capitulos.
...
if (yaHayDatos > 0) {
  console.log('Ya hay capitulos cargados, no se inserta nada (para no duplicar).');
  process.exit(0);
}
